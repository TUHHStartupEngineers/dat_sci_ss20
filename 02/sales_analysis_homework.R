...
order_items_joined_tbl  <- order_items_tbl %>% 
  ...
  left_join(translations_tbl, by = c("product.category.name" = "product_category_name"))
...
order_items_wrangled_tbl <- order_items_joined_tbl %>%
  ...
  # Separate product category name in main and sub (translations)
  separate(col    = product_category_name_english,
           into   = c("main.category.name.english", "sub.category.name.english"),
           sep    = " - ",
           remove = FALSE) %>%
  
  # Separate location in city and state
...
# 8.1 Sales by seller loaction ----
library(lubridate)
# Step 1 - Manipulate

revenue_by_location_tbl <- order_items_wrangled_tbl %>% 
  
  # Select columns
  select(seller_state, total_price) %>% 
...
revenue_by_location_tbl %>% 
  
  # Setup canvas with year (x-axis) and revenue (y-axis)
  ggplot(aes(x = seller_state, y = revenue)) + 
  ...
  scale_y_continuous(labels = scales::dollar) +
  labs(
    title    = "Revenue by seller location",
    subtitle = "Upward Trend",
    x = "Seller location",
    y = "Revenue"
  )

# 8.2 Sales by seller loaction and year ----
# Step 1 - Manipulate
revenue_by_year_location_tbl <- order_items_wrangled_tbl %>% 
  
  # Select columns
  select(order_date, total_price, seller_state) %>% 
  mutate(year = year(order_date)) %>% 
  
  # Filter  > 100.000
  group_by(seller_state) %>% 
  filter(sum(total_price) > 100000) %>% 
  ungroup() %>% 
...
revenue_by_year_location_tbl %>% 
  ...
  scale_y_continuous(labels = scales::dollar) +
  labs(
    title = "Revenue by year and seller location",
    subtitle = "Each location has an upward trend",
    fill = "Seller state"
  )

# 8.5 Analysis with translations ----
library(readxl)
translations_tbl <- read_excel("00_data/01_e-commerce/01_raw_data/product_category_name_translation.xlsx")